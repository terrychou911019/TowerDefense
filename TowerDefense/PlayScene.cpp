...
void PlayScene::Initialize() {
    ...
    timeleft_timer = al_create_timer(1.0);
    //al_set_timer_count(timeleft_timer, 0);
    al_start_timer(timeleft_timer);
    AddNewObject(UITimeLeft = new Engine::Label("Time Left: " + std::to_string(40 - al_get_timer_count(timeleft_timer)), "pirulen.ttf", 40, 200, 50, 0, 0, 0, 255, 0.5, 0.5));
...
void PlayScene::Update(float deltaTime) {
    ...
    //shoe the time
    UITimeLeft->Text = "Time Left: " + std::to_string(40 - al_get_timer_count(timeleft_timer));

    //timesout or not
    bool timesout = false;
    if (al_get_timer_count(timeleft_timer) >= 40.0) {
        al_stop_timer(timeleft_timer);
        al_set_timer_count(timeleft_timer, 0);
        timesout = true;
    }
    // Win
    ...
    if (you_win == true) Engine::GameEngine::GetInstance().ChangeScene("win");
...
void PlayScene::OnMouseUp(int button, int mx, int my) {
	...
	if(x < 0 || x >= MapWidth || y < 0 || y >= MapHeight)
	    return;
	if (button & 1) {
		if (!CheckOccupied(x, y) || preview && preview->id == 3) {
			if (!preview)
				return;
...
void PlayScene::OnKeyDown(int keyCode) {
	...
	if (keyCode == ALLEGRO_KEY_Q) {
        ...
        if (keyCode == ALLEGRO_KEY_0) 
            al_set_timer_speed(timeleft_timer, 10000000000000000000.0);
        else
...
void PlayScene::ReadMap() {
	...
	while (fin >> c) {
        ...
        case '3': mapData.push_back(TILE_BLUE_CANNON); break;
		...
		for (int j = 0; j < MapWidth; j++) {
            ...
            switch (md) {
                ...
                case TILE_FLOOR:
                    ...
                    break;
                case TILE_BLUE_CANNON:
                    DefenseGroup->AddNewObject(new BlueCannonDefense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                    break;
                case TILE_TRAP:
